...
import { DatePicker, message, Select, Button, Table } from 'antd';
...
class App extends React.Component {
	constructor(props) {
		...
		this.state = {
			giftID: '',
			startTime: today.toLocaleDateString().replaceAll('/', '-'),
			endTime: today.toLocaleDateString().replaceAll('/', '-'),
			giftTypeList: [],
			cookie: '',
			inputValue: '',
			giftID: '',
	...
	setCookie = () => {
		...
		this.setState({ cookie: this.state.inputValue }, () => {
			this.getGiftTypeList();
		});
	};
	setGiftTypeId = (id) => {
	...
	ExportToExcel = () => {
		var option = [];
		const { startTime, endTime, exportData } = this.state;
		let sheetfilter = ['UID', '昵称', '礼物类型', '赠送数量', '礼物价值', '赠送时间']; //对应列表数据中的key值数组，就是上面resdata中的 name，address
		let sheetheader = ['UID', '昵称', '礼物类型', '赠送数量', '礼物价值', '赠送时间']; //对应key值的表头，即excel表头

		option.fileName = '礼物流水表' + startTime + '至' + endTime; //导出的Excel文件名
		option.datas = [
			{
		...
		return (
			<div className='App'>
				<div style={{ textAlign: 'left', width: '800px' }}>
					...
					<Input
						style={{ width: '300px' }}
						placeholder='请设置SESSDATA Cookie'
						onChange={this.onChange}
					/>
					<Button className='btn' onClick={this.setCookie}>
						登录
					</Button> */}
					<br />
					<label>礼物类型：</label>
					<Select style={{ width: 100 }} onSelect={this.setGiftTypeId} defaultValue=''>
					...
					<RangePicker
						defaultValue={[
							moment(today.toLocaleDateString(), 'YYYY/MM/DD'),
							moment(today.toLocaleDateString(), 'YYYY/MM/DD')
						]}
						format='YYYY-MM-DD'
						onChange={this.timeOnChange}
					/>
					<Button className='btn' onClick={this.getAllGiftList}>
						查询
					</Button>
					<Button onClick={this.ExportToExcel}>导出</Button>
					<Table
						dataSource={giftList}
						columns={columns}
						loading={loading}
